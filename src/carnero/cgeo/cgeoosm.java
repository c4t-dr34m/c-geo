...
public class cgeoosm extends Activity {
	...
	// overlays
	private cgOMapOverlay overlayCaches = null;
	private cgOUsersOverlay overlayUsers = null;
	private cgOOverlayScale overlayScale = null;
	private cgOMapMyOverlay overlayMyLoc = null;
	// data for overlays
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (overlayUsers == null) {
			overlayUsers = new cgOUsersOverlay(activity, getResources().getDrawable(R.drawable.user_location));
			overlays.add(overlayUsers);
		}

		if (overlayMyLoc == null) {
			overlayMyLoc = new cgOMapMyOverlay(activity, settings);
			overlays.add(overlayMyLoc);
		}

		if (overlayCaches == null) {
			overlayCaches = new cgOMapOverlay(activity, getResources().getDrawable(R.drawable.marker), fromDetailIntent);
			overlays.add(overlayCaches);
		}

		if (overlayScale == null) {
			overlayScale = new cgOOverlayScale(activity, base, settings);
		...
		// google analytics
		if (live) {
			base.sendAnal(activity, "/osm/live");

			followMyLocation = true;
		} else {
			base.sendAnal(activity, "/osm/normal");
	...
	private class DisplayThread extends DoThread {
		...
		public void run() {
			...
			final ArrayList<cgOOverlayItem> items = new ArrayList<cgOOverlayItem>();

			if (cachesProtected != null && !cachesProtected.isEmpty()) {
				int counter = 0;
				int icon = 0;
				Drawable pin = null;
				cgOOverlayItem item = null;
	...
	private class DisplayUsersThread extends DoThread {
		...
		public void run() {
			...
			// display users
			ArrayList<cgOOverlayUser> items = new ArrayList<cgOOverlayUser>();

			int counter = 0;
			cgOOverlayUser item = null;
			...
			for (cgUser userOne : users) {
				...
				item = new cgOOverlayUser(activity, userOne);
				items.add(item);
	...
	private class DisplayPointThread extends Thread {
		...
		public void run() {
			...
			if (latitudeIntent != null && longitudeIntent != null) {
				...
				coordinates.add(coord);
				cgOOverlayItem item = new cgOOverlayItem(coord);
